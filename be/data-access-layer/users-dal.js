...
router.get('/user',verifytoken.user ,(req, res) => {
    const con = app.get('CONNECTION');
    let sql = `SELECT * FROM users WHERE i_d = ${req.auth.user_id}`
    con.query(sql, (err, result, fields) => {
        if (err) {
            res.json({ state: 'error', message: err.message })
        } else {
...
router.post('/verifidmail',(req,res)=>{
    const con = app.get('CONNECTION');
    let sql = `SELECT * FROM users WHERE i_d=${req.body.user_id} OR mail='${req.body.email}'`
    con.query(sql, (err, result, fields) => {
        if (err) {
            res.json({ state: 'error', message: err.message })
        } else {
...
router.post('/add',async (req,res)=>{
    ...
    if (!user_id || !first_name || !last_name || !email || !password || !city || !adress) {
        ...
        const con = app.get('CONNECTION');
        sql = `INSERT INTO users(i_d, first_name, last_name, mail, password_hach, city, adress)
    ...
    con.query(sql, async (err, result, fields) => {
        if (err) {
            ...
            if (result.length > 0) {
                ...
                if (valid_password) {
                    // jwt.sign({ mail: result[0].mail, admin: result[0].admin, first_name:result[0].first_name, last_name:result[0].last_name, user_id:result[0].i_d }, 'secretkey', { expiresIn: "1m" }, (err, token) => {
                    jwt.sign({ mail: result[0].mail, admin: result[0].admin, first_name:result[0].first_name, last_name:result[0].last_name, user_id:result[0].i_d }, 'secretkey', (err, token) => {
...
router.get('/generalInformation',(req, res) => {
    ...
    con.query(sql, (err, result, fields) => {
        if (err) {
            ...
            console.log(result)
            let countProducts=0,countOrders=0
            if(result[0]){countProducts = result[0].countInfo}
            if(result[1]){countOrders = result[1].countInfo}else{countOrders=result[0].countInfo}
            res.json({ state: 'success', message: {countProducts, countOrders} })
            // if (result.length > 0) {
            //     if(result[0])
            //     res.json({ state: 'success', message: result })
            // } else {
